// FILE: src/routes/sveltepress/lib/LayoutHelper.ts
...
export function sveltePressLayoutHelper(
  ...
  if(routeId === undefined || routeId === null) {
    ...
    debugWarn('[SveltePress] routeId is undefined or null, returning default layout state.');
    return {
      showHeader: true,
      showFooter: true,
    };
  ...
  const isPreviewRoute =
    routeId?.includes('/sveltepress/preview') || url.searchParams.has('preview');

  const isEditorRoute = routeId?.includes('/sveltepress/editor');
  if (isPreviewRoute || isEditorRoute) {
    state.showHeader = false;
    // Per your request, the footer is always shown.
...
export function scanAndReportComponents() {
  ...
  setTimeout(() => {
    ...
    debugLog(
      `[SveltePress] Scanned page and found ${components.length} components for navigation.`,
      components,
    );
    ...
    window.parent.postMessage(
      {
        type: 'SVELTEPRESS_PAGE_COMPONENTS',
        components,
      },
 ...
 * Initializes client-side SveltePress functionality like click-to-edit
 * and page scanning for the editor.
 */
export function initializeSveltePressClient() {
  if (!browser) return;
  const currentPage = get(page);

  // If we're on a SveltePress admin route, do nothing.
  if (currentPage.route.id?.startsWith('/sveltepress')) {
    return;
  }
  ...
  const handleEditClick = (event: MouseEvent) => {
    ...
    debugLog(
      `[SveltePress] Ctrl+Alt+click detected. Initial target nodeType: ${target.nodeType}`,
    );
    ...
    if (target.nodeType === 3) {
      ...
      debugLog(
        '[SveltePress] Target was a text node. Using parent element to start search:',
        searchStartElement,
      );
      ...
      debugLog(
        '[SveltePress] Target was an element. Starting search from:',
        searchStartElement,
      );
    } else {
    ...
    if (!searchStartElement) {
      debugError(
        '[SveltePress] Could not determine a valid starting element for search.',
      );
      return;
    }
    ...
    if (wrapper) {
      ...
      if (componentPath) {
        debugLog(`[SveltePress] Found component wrapper: ${componentPath}`);
        ...
        if (editableEl instanceof HTMLElement) {
          debugLog('[SveltePress] Found editable element:', editableEl);
          const selector = getSelector(editableEl);
          debugLog(`[SveltePress] Generated selector: "${selector}"`);

          const returnToPath = window.location.pathname;
          const finalUrl = `/sveltepress/editor?component=${encodeURIComponent(
            componentPath,
          )}&selector=${encodeURIComponent(
            selector,
          )}&returnTo=${encodeURIComponent(returnToPath)}`;
          debugLog(`[SveltePress] Redirecting to: ${finalUrl}`);
          window.location.href = finalUrl;
        } else {
          debugError(
            '[SveltePress] Could not find a valid editable container element (p, h1, etc.).',
          );
        }
      }
    } else {
      debugLog(
        '[SveltePress] Click was not inside a data-sp-component wrapper.',
      );
    }
  };
