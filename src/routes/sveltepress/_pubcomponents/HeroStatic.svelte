...
const PLACEHOLDER_IMAGE_SRC = `data:image/svg+xml,${encodeURIComponent(
  placeholderSvg,
)}`;
  ...
  $: {
    if (slides && slides.length > 0) {
      ...
      if (newSlide?.headingPhrase !== currentSlide?.headingPhrase || newSlide?.bgImageSrc !== currentSlide?.bgImageSrc) {
      ...
      } else if (newSlide && currentSlide && JSON.stringify(newSlide.bgImageSet) !== JSON.stringify(currentSlide.bgImageSet)) {
  ...
  // --- Logic ---
  function selectBestImageSource(
    slide: HeroSlide,
    width: number,
  ): string {
  ...
  function updateDisplayedImage(slide: HeroSlide, width: number, currentDisplayedSrc: string) {
    ...
    if (!targetSrc) {
        ...
        displayedBgImage = '';
        if (imageLoader) {
            imageLoader.onload = null; imageLoader.onerror = null; imageLoader = null;
        }
        return;
    }
...
<style>
  ...
  .hero-static-container {
    position: relative;
    min-height: 54.3vh; /* Adjust as needed */
