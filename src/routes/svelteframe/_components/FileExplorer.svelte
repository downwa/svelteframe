<!-- FILE: src/routes/svelteframe/_components/FileExplorer.svelte -->
<script lang="ts">
	...
	function buildTree(files: FileEntry[], type: 'page' | 'component'): FileTree {
		const tree: FileTree = {};
		const prefix = type === 'page' ? 'src/routes/' : 'src/lib/';
	...
	async function handleContextMenu(event: MouseEvent, file: FileEntry) {
		...
		try {
			const res = await fetch(
				`/svelteframe/api/files/backups?path=${encodeURIComponent(file.path)}`
			);
			if (res.ok) {
					...
					{:else}
						<li class="folder-item">
							<button class="folder-header" on:click={() => toggleFolder(folder)}>
								<span class="icon" class:expanded={expandedFolders[folder]}> ▶ </span>
								{folder}
							</button>
