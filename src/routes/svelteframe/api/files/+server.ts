// FILE: src/routes/svelteframe/api/files/+server.ts
...
export async function GET({ locals }) {
  ...
  allRouteFiles.forEach((p) => {
    const issvelteframeFile = p.replace(/\\/g, '/').startsWith('src/routes/svelteframe');
    ...
    if (issvelteframeFile || isBackupFile) {
      return;
    }

    // AUTH FILTERING: Check if user has read permission for this file
    const webPath = p.replace(/\\/g, '/');
    if (locals.user && !hasPermission(locals.user, webPath, 'R')) {
      return;
    }
  ...
  const filteredLibComponents = allLibComponents.filter(
    (p) => {
      if (path.basename(p).startsWith('backup-')) return false;
      // AUTH FILTERING
      const webPath = p.replace(/\\/g, '/');
      if (locals.user && !hasPermission(locals.user, webPath, 'R')) {
        return false;
