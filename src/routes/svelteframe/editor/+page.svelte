<script lang="ts">
	...
	// --- Permissions Logic ---
	// Retrieve pre-calculated permissions from the layout's user object
	...
	$: canGoToNextComponent =
		pageComponents.length > 0 && currentComponentIndex < pageComponents.length - 1;
	...
	$: {
		...
		if (componentFromUrl && !isLoadingFiles && editorPane) {
			handleEditRequest(componentFromUrl, $page.url.searchParams.get('selector') || undefined);
		}
	}
	...
	async function handleSave() {
		if (editorPane) {
			...
			window.parent.postMessage(
				{
					type: 'svelteframe_EXIT_SURGICAL_EDIT',
	...
	function navigateComponent(direction: 'next' | 'prev') {
		...
		if (newIndex >= 0 && newIndex < pageComponents.length) {
			...
			const newUrl =
				`/svelteframe/editor?component=${encodeURIComponent(component.path)}` +
	...
	function navigateTag(direction: 'next' | 'prev') {
		...
		if (newIndex >= 0 && newIndex < availableTags.length) {
			...
			console.log(
				`[editor+P:111] Navigating to path: ${activeEditingFile.path} with tag: ${selector}`
			);
	...
	function handleEditRequest(componentPath: string, selector?: string) {
		...
		if (fileToEdit) {
			...
			if (selector && editorPane) {
				console.log(`[editor+P:122] Navigating to path: ${fileToEdit.path} with tag: ${selector}`);
	...
	onMount(async () => {
		...
		const handleMessage = (event: MessageEvent) => {
			const { type, tags, components } = event.data;
			...
			if (type === 'svelteframe_CONTENT_DISCOVERED') {
			...
			} else if (type === 'svelteframe_PAGE_COMPONENTS') {
				...
				if (currentComponentPath) {
					const index = pageComponents.findIndex((c) => c.path.includes(currentComponentPath));
					if (index !== -1) {
...
{#if scanUrl}
	<iframe
		...
		title="SvelteFrame Page Scanner"
